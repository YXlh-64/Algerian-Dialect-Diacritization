...
def _brute_force(crf, em, is_intra, b, K, T):
    ...
    for seq in itertools.product(range(K), repeat=T):
        ...
        scores.append(s)
        if s > best[0]:
            best = (float(s.detach()), list(seq))
    log_partition = torch.logsumexp(torch.stack(scores), 0)
    return float(log_partition.detach()), best[1]
...
def test_crf_split_transitions():
    ...
    for b in range(B):
        ...
        lz_crf = float(crf._logZ(em, mask, ii)[b].detach())
...
def test_crf_ignores_padding():
    ...
    lz_pad = float(crf._logZ(em, mask, ii)[1].detach())
    lz_solo = float(crf._logZ(em_s, mask_s, ii_s)[0].detach())
