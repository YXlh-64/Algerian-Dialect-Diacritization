...
from models.track4.AbidatSouha.crf import is_intra_mask
from utils.track4.AbidatSouha.data import collate
...
def load_test_set(paths, data):
    ...
    """
    with Path(paths.raw_test_ids).open(encoding="utf-8") as stream:
        sent_ids = [line.strip() for line in stream]
    with Path(paths.raw_test).open(encoding="utf-8") as stream:
        raw_lines = [re.sub(r"\s+", " ", line.strip()) for line in stream]
    if len(sent_ids) != len(raw_lines):
        raise ValueError("ids / sentences length mismatch")
