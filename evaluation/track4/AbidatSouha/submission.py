...
by design -- report the scores in documentation/track4/AbidatSouha/README.md instead.
"""
...
def verify_against_sample(rows, sample_path, verbose=True):
    ...
    if not sample_path or not os.path.exists(sample_path):
        return rows
    with Path(sample_path).open(encoding="utf-8") as stream:
        want = [row[0] for row in list(csv.reader(stream))[1:]]
    got = [r[0] for r in rows]
    if len(want) != len(got):
        raise ValueError(f"row count: expected {len(want)}, got {len(got)}")
    if set(want) != set(got):
        raise ValueError("Id set does not match sample_submission.csv")
