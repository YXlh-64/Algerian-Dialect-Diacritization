...
def continue_finetune(m, ckpt_dir: Path, base_train_records: List[dict], pseudo_records: List[dict],
    ...
    Two overfitting fixes vs. the original version:
      1. Snapshots `m`'s weights before any self-training happens. If no
         self-training epoch ever beats `start_score`, `m` is restored to
         that exact snapshot. Previously, a run that never improved still
         left `m` holding whatever the LAST epoch produced (the most
         overfit state) even though it was logged as "kept original" --
         that overfit state was then silently carried into the final
         ensemble. This is what caused the measured DEV_TEST regression
         (0.9483 -> 0.9430) after self-training.
      2. Early-stops on val_loss plateau using the same criterion/patience
         as main training (CFG.early_stop_metric/early_stop_patience),
         instead of always running the full CFG.self_training_epochs. A
         fold that starts overfitting on epoch 1 no longer keeps training
         for 3 more epochs regardless.

    Returns (best_val_acc_achieved, improved_over_start_score).
    '''
    pre_selftrain_state = copy.deepcopy(m.state_dict())
    ...
    st_ckpt = CheckpointManager(ckpt_dir, save_optimizer=False)  # short run, resume not worth the disk cost
    state = st_ckpt.load_latest(m, opt, sched)
    step, best_acc = state["global_step"], start_score
    best_val_loss, st_patience = float("inf"), 0
    ...
    for epoch in range(state["epoch"], CFG.self_training_epochs):
        ...
        overfit_flag = " <- val_loss rising (overfitting signal)" if v_loss > best_val_loss else ""
        print(f"    epoch {epoch+1}/{CFG.self_training_epochs} | train_acc={tr_acc:.4f} "
              f"| val_loss={v_loss:.4f} val_acc={v_acc:.4f}{overfit_flag}")
        is_best = v_acc > best_acc
        st_ckpt.save(m, opt, sched, epoch + 1, step, max(best_acc, v_acc), is_best=is_best)
        if is_best:
            best_acc = v_acc

        improved_loss = v_loss < best_val_loss
        if improved_loss:
            best_val_loss = v_loss
        st_patience = 0 if improved_loss else st_patience + 1
        if st_patience >= CFG.early_stop_patience:
            print(f"    self-training early stop (val_loss plateaued for {CFG.early_stop_patience} epochs).")
            break
    ...
    if improved:
        ...
        # Nothing in this self-training run beat the pre-self-training model --
        # restore it exactly rather than leaving `m` at its last (most overfit) epoch.
        m.load_state_dict(pre_selftrain_state)
