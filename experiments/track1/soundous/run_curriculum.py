"""BiLSTM-CNN-CRF + length-based curriculum learning (shortest-first for the first few
epochs, then normal bucketed shuffling).
"""
...
def main():
    ctx = load_everything()
    cfg = load_config("curriculum")
    out_dir = os.path.join(ctx["paths"]["checkpoints_dir"], "exp_curriculum")

    train_curriculum(
        ctx["vocab_size"], ctx["num_classes"], ctx["pad_idx"], ctx["unk_idx"], ctx["device"],
        ctx["model_kwargs"], ctx["train_rows"], ctx["char2idx"], ctx["label2idx"], ctx["no_diac_idx"],
